...
DEFAULT_FN_MODEL = './_train/character_pose_estim/runs/feat_concat.ckpt'
FN_MODEL = DEFAULT_FN_MODEL
...
def run_single_retrieval(fn_img: str):
    """
    Run the full pose estimation + nearest neighbors retrieval pipeline
    for a single image file.

    Returns a dictionary with:
        - input_path: the input image path
        - bbox: bounding box [[x_min, y_min], [w, h]]
        - keypoints: numpy array of keypoints in original image coordinates
        - cropbox: cropbox used to normalize pose
        - neighbors_post_ids: list of Danbooru post IDs (length == number of neighbors)
        - neighbors_distances: 1D array of distances corresponding to each neighbor
    """
    img = I(fn_img)
    ans = infer_pose(model_pose, model_segmenter, [img,])
    result = ans[0]

    dist, idx = retrieve_nearest(nbrs, result['keypoints'], result['cropbox'])
    post_ids = [bns[v] for v in idx[0]]
    distances = dist[0]
...
def main():
    ...
    result = run_single_retrieval(args.fn_img)
    ...
    # keypoints
    print('keypoints')
    for k, (x, y) in zip(ukey.coco_keypoints, result['keypoints']):
        print((f'\t({x:.2f}, {y:.2f})'), k)
    print()
    ...
    print(Table([
        ...
        *[
            [f'{i: 2d}', f'{dt:.2f}::r', f'https://danbooru.donmai.us/posts/{pid}::l']
            for i, (pid, dt) in enumerate(zip(post_ids, distances))
        ],
    ]))
